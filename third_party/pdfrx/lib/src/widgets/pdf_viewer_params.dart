...
class PdfViewerParams {
  const PdfViewerParams({
    ...
    @Deprecated(
      'Use sizeDelegateProvider: PdfViewerSizeDelegateProviderLegacy(maxScale: ...) instead',
    )
    this.maxScale,
    @Deprecated(
      'Use sizeDelegateProvider: PdfViewerSizeDelegateProviderLegacy(minScale: ...) instead',
    )
    this.minScale,
    @Deprecated(
    ...
    this.pagePaintFilterQuality = FilterQuality.low,
    this.visibleRenderScaleFactor = 1.0,
    this.additionalRenderFlags = PdfPageRenderFlags.none,
    ...
    @Deprecated(
      'Use sizeDelegateProvider: PdfViewerSizeDelegateProviderLegacy(calculateInitialZoom: ...) instead',
    )
    ...
    this.zoomStepsDelegateProvider =
       ...
       assert(
         visibleRenderScaleFactor >= 1.0,
         'visibleRenderScaleFactor must be at least 1.0',
       );
  ...
  /// Filtering used when cached page images are composited onto the viewer.
  ///
  /// [FilterQuality.medium] generally produces clearer text when a rasterized
  /// page is displayed at a fractional scale.
  final FilterQuality pagePaintFilterQuality;

  /// Extra render resolution used only for the visible partial page image.
  ///
  /// Full-page previews are unaffected, so modest values can improve text
  /// clarity without multiplying the cache cost for every nearby page.
  final double visibleRenderScaleFactor;

  /// Additional [PdfPageRenderFlags] passed to both preview and partial renders.
  final int additionalRenderFlags;
  ...
  bool doChangesRequireReload(PdfViewerParams? other) {
    return other == null ||
        ...
        other.pagePaintFilterQuality != pagePaintFilterQuality ||
        other.visibleRenderScaleFactor != visibleRenderScaleFactor ||
        other.additionalRenderFlags != additionalRenderFlags ||
  ...
  int get hashCode {
    return margin.hashCode ^
        ...
        pagePaintFilterQuality.hashCode ^
        visibleRenderScaleFactor.hashCode ^
        additionalRenderFlags.hashCode ^
        pageAnchor.hashCode ^
...
/// }
/// ```
typedef PdfDocumentLoadFinished =
    void Function(PdfDocumentRef documentRef, bool loadSucceeded);
