...
class CustomPlatformViewController
  ...
  Future<void> _setScrollDelta(double dx, double dy,
      {required bool isPrecision, required int source}) async {
    if (_isDisposed) {
      return;
    }
    assert(value.isInitialized);
    return _methodChannel
        .invokeMethod('setScrollDelta', [dx, dy, isPrecision, source]);
  }
                ...
                ? Listener(
                    ...
                    onPointerSignal: (signal) {
                      if (signal is PointerScrollEvent) {
                        ...
                        final values =
                            [delta.dx, delta.dy].where((value) => value != 0.0);
                        final isPrecision =
                            signal.kind == PointerDeviceKind.trackpad ||
                                values.any((value) {
                                  final magnitude = value.abs();
                                  return magnitude < 19.5 ||
                                      (magnitude % 20.0) > 0.01;
                                });
                        _controller._setScrollDelta(-delta.dx, -delta.dy,
                            isPrecision: isPrecision, source: 0);
                      }
                    },
                    onPointerPanZoomUpdate: (event) {
                      _controller._setScrollDelta(
                          event.panDelta.dx, event.panDelta.dy,
                          isPrecision: true, source: 1);
                    },
                    child: MouseRegion(
                        cursor: _cursor,
                        child: Texture(
