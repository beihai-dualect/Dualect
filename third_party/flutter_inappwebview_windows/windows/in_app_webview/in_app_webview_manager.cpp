...
namespace flutter_inappwebview_plugin
{
  ...
  void InAppWebViewManager::createInAppWebView(const flutter::EncodableMap* arguments, std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result)
  {
    ...
    const auto flutterViewHwnd = plugin->registrar->GetView()->GetNativeWindow();
    const auto flutterWindowHwnd = GetAncestor(flutterViewHwnd, GA_ROOT);
    RECT bounds;
    GetClientRect(flutterViewHwnd, &bounds);

    // The composition host is positioned in screen coordinates, so it must be
    // an owned top-level window rather than a child of the Flutter view. Using
    // the actual app window as its owner makes Windows hide it automatically
    // when Dualect is minimized; otherwise the invisible WebView host can keep
    // intercepting clicks over the desktop.
    auto hwnd = CreateWindowEx(WS_EX_NOACTIVATE | WS_EX_TOOLWINDOW,
      windowClass_.lpszClassName, L"", WS_POPUP, 0,
