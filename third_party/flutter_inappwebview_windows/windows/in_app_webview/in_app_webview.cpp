...
namespace flutter_inappwebview_plugin
{
  ...
  // flutter_view
  void InAppWebView::setSurfaceSize(double width, double height, double scale_factor)
  {
    ...
    if (surface_ && width > 0 && height > 0) {
      scaleFactor_ = static_cast<float>(scale_factor);
      const auto scaled_width = static_cast<LONG>(std::lround(width * scale_factor));
      const auto scaled_height = static_cast<LONG>(std::lround(height * scale_factor));

      RECT bounds;
      bounds.left = 0;
      bounds.top = 0;
      bounds.right = scaled_width;
      bounds.bottom = scaled_height;
      ...
      if (surfaceSizeChangedCallback_) {
        surfaceSizeChangedCallback_(
          static_cast<size_t>(std::lround(width)),
          static_cast<size_t>(std::lround(height)));
      }
    }
  }


  void InAppWebView::setPosition(double x, double y, double scale_factor)
  {
    ...
    if (x >= 0 && y >= 0) {
      scaleFactor_ = static_cast<float>(scale_factor);
      const auto scaled_x = static_cast<int>(std::lround(x * scale_factor));
      const auto scaled_y = static_cast<int>(std::lround(y * scale_factor));
    ...
    bool isPrecision)
  {
    ...
    // A mouse wheel normally arrives in 120-unit steps, while precision
    // touchpads emit many small fractional deltas. Preserve the remainder
    // between events instead of truncating slow gestures to zero.
    // Transport input to Chromium without deciding the document's reading
    // speed here. Discrete wheel input is converted to WHEEL_DELTA while
    // precision input keeps its pixel-like delta for the page controller.
    constexpr double kMouseWheelMultiplier = 6.0;
    constexpr double kPrecisionTouchpadMultiplier = 6.0;
    const double scrollMultiplier = isPrecision
      ? kPrecisionTouchpadMultiplier
      : kMouseWheelMultiplier;
    auto& remainder = horizontal ? horizontalScrollRemainder_
                                 : verticalScrollRemainder_;
    const double scaledDelta = delta * scrollMultiplier + remainder;
    const double integralDelta = std::trunc(scaledDelta);
    if (integralDelta == 0.0) {
      remainder = scaledDelta;
      return;
    }
    const double boundedDelta = std::clamp(
      integralDelta, static_cast<double>(SHRT_MIN),
      static_cast<double>(SHRT_MAX));
    remainder = scaledDelta - boundedDelta;
    const auto offset = static_cast<short>(boundedDelta);
    ...
    bool isPrecision)
  {
    ...
    if (delta_x != 0.0) {
      sendScroll(delta_x, true, isPrecision);
    }
    if (delta_y != 0.0) {
      sendScroll(delta_y, false, isPrecision);
    }
  }

  void InAppWebView::scrollBy(double delta_x, double delta_y) const
  {
    if (!webView) {
      return;
    }

    const auto source = "window.dualectPanScroll && "
      "window.dualectPanScroll(" + std::to_string(delta_x) + "," +
      std::to_string(delta_y) + ");";
    webView->ExecuteScript(utf8_to_wide(source).c_str(),
