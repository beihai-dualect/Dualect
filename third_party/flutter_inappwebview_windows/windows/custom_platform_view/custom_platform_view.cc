...
namespace flutter_inappwebview_plugin
{
    ...
    std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result)
  {
    ...
    // setScrollDelta: [double dx, double dy, bool isPrecision, int source]
    if (method_name.compare(kMethodSetScrollDelta) == 0) {
      ...
      const auto isPrecision = std::get_if<bool>(&(*list)[2]);
      const auto source = std::get_if<int32_t>(&(*list)[3]);
      if (dx && dy && isPrecision && source && view) {
        if (*source == 1) {
          view->scrollBy(*dx, *dy);
        }
        else {
          view->setScrollDelta(*dx, *dy, *isPrecision);
        }
        return result->Success();
      }
      return result->Error(kErrorInvalidArgs);
    ...
    if (method_name.compare(kMethodSetSize) == 0) {
      ...
      if (size && view) {
        ...
        view->setSurfaceSize(width, height, scale_factor);
      ...
      if (position && view) {
        ...
        view->setPosition(x, y, scale_factor);
